...
def build_prompt(sources: list[dict], state: dict, today: str) -> str:
    ...
    lines.extend([
        "## 写作要求",
        "",
        "- markdown 报告：按厂商分组（## 腾讯 WorkBuddy / ## 字节 Coze / ...），每组下用 bullet 列新增条目",
        "- 顶部加一个 '## 摘要' 节，2-3 句概括今日赛道动态",
        "- **重大信号** 节单独列在摘要之后，每条带 [vendor] 标签",
        "- 引用必须带 URL",
        "- 没有新动态的源也要简要说明 '无明显更新'，不要省略",
        "- 全文中文、专业、不口语化",
        "- **JSON 必须可被 json.loads 解析**，字符串里的换行用 \\n",
    ])
